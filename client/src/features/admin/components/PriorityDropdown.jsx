...
export function PriorityDropdown({ currentPriority, onUpdatePriority, isUpdating }) {
  const [selectedPriority, setSelectedPriority] = useState(currentPriority || "medium");

  const handleUpdate = async () => {
    if (selectedPriority !== currentPriority) {
      await onUpdatePriority(selectedPriority);
      // selectedPriority will update via currentPriority prop after mutation completes
    }
  };
  ...
  const currentOption = getCurrentOption(currentPriority || "medium");
  const selectedOption = getCurrentOption(selectedPriority);
  const isChanged = selectedPriority !== currentPriority;
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        >
          {getCurrentOption(currentPriority || "medium").label}
        </div>
      </div>
